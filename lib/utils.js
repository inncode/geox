        ...
        return new Promise((res, rej) => {
            if (utils.checkGeoliteCityExists()) {
                ...
                inquirer.prompt([{
                    type: "list",
                    name: "options",
                    message: "To use ip utils you need the MaxMind GeoLite2 City database, you want to download?",
                    choices: ['Yes', 'No']
                }]).then((r) => {
                    if (r.options === 'No')
                        return rej('The GeoLite2City is required!');

                    const status = new Spinner('Downloading GeoLite2 City, please wait...');
                    status.start();
                    utils.downloadGeoliteCity().then((rdown) => {
                        ...
                        status.stop();
                        rej('Failed to get GeoLite2 City.');
                    });
                }).catch(() => rej('The GeoLite2 City is required!'));
            }
        })
    },
