    ...
    licenses: () => {
        return new Promise((res, rej) => {
            if(utils.checkGeoliteCityExists()) {
                console.log(chalk.green('[IP DataBase] '), 'This product includes GeoLite2 data created by MaxMind, available from https://www.maxmind.com');
            }
            console.log(chalk.green('[Geolocation DataBase] '), 'Data © OpenStreetMap contributors, ODbL 1.0. https://osm.org/copyright');
            ...
            inquirer.prompt([{
                ...
                utils.searchLocation(`${r.lat},${r.lon}`).then((rws) => {
                    ...
                    for (let i = 0; i < locates.length; i++) {
                        console.log(chalk.blue('[Address] > '), locates[i].display_name, chalk.yellow('License: ' + locates[i].licence));
                    }
                    res();
                }).catch((e) => {
                    status.stop();
        ...
        return new Promise((res, rej) => {
            inquirer.prompt({
                ...
                utils.searchLocation(`${r.address}`).then((rws) => {
                    ...
                    let locates = JSON.parse(rws.body);
                    for (let i = 0; i < locates.length; i++) {
                        console.log(chalk.blue('[Geoposition] > '), `${locates[i].lat},${locates[i].lon}`, chalk.yellow('License: ' + locates[i].licence));
                    }
                    res();
                }).catch((r) => {
                    status.stop();
